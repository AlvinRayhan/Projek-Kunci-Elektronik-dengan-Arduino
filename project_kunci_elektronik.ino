//Pertemuan 10 : Kunci Elektronik dengan Password
//Alvin Rayhan Jayusman (TT3B)
...
//inisialisasi untuk keypad atau definsikan keymap
...
Keypad passpad = Keypad( makeKeymap(keys), PinBaris,   PinKolom, JmlBaris, JmlKolom ); 
...
void setup() {
  ...
  delay(500);
  lcd.clear();
  passpad.addEventListener(keypadEvent);
}
...
void loop() {  
  ...
  passpad.getKey(); //membaca keypad
}

void keypadEvent(KeypadEvent eKey){ 
  switch (passpad.getState())
  {
